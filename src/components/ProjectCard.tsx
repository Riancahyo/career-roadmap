...
export const ProjectCard = ({ project, onClick }: ProjectCardProps) => {
  ...
  return (
    <motion.div
      onClick={onClick}
      className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:border-white/30 hover:bg-white/10 transition-all cursor-pointer group relative"
      whileHover={{ y: -5 }}
      whileTap={{ scale: 0.98 }}
    >
      {/* Status Badge */}
      {status !== 'not-started' && (
        <div className="absolute top-4 right-4">
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
            ...
            {status === 'completed' ? '✓ Completed' : `${progress}% Done`}
          </span>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 pr-20">
          <h3 className="text-xl font-bold text-white mb-2 group-hover:text-[#ffaa40] transition-colors">
            {project.title}
          </h3>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/10">
        ...
        {project.participants && (
          <div className="flex items-center gap-1 text-white/60 text-sm">
            ...
            <span>{project.participants.toLocaleString()} Started</span>
          </div>
        )}
      </div>
    </motion.div>
